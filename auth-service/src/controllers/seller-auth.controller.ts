...
export class SellerAuthController {
  ...
  async signIn(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Check seller status - only suspended accounts are blocked
      // Missing seller record or service failure should not block login (features restricted on frontend)
      if (sellerData && sellerData.status === 'suspended') {
